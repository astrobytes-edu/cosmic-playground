...
next: the stage-first campaign is DONE for readouts -- no demo has a readout below the fold at 1440x900, and retrograde-motion, the last one, is also clean at 1920x1080, 1366x768 and 1280x720. Out of the BUDGETS map entirely: telescope-resolution, conservation-laws, planetary-conjunctions, blackbody-radiation, eclipse-geometry, seasons, spectral-lines, retrograde-motion. What remains is SIDEBAR overflow, measured at 1440x900: parallax-distance (920px), keplers-laws (929), eos-lab (508), binary-orbits (330), galaxy-rotation (148), doppler-shift (60), stars-zams-hr (25), spectral-lines/retrograde-motion (0 at 1440, 113/69 at 1280x720). The eos-lab and keplers-laws figures rose on 2026-09-05 without either demo getting worse -- see the accordion note below; the content was always there, collapsed to a 2px box. No demo now gates its stage rules on viewport HEIGHT. KNOWN, NOT MINE: two binary-orbits `visual` baselines (`binary-orbits-rv-inclination-30`, `binary-orbits-rv-sb1`) fail on the `#rvPanel` canvas; verified 2026-09-05 that they fail identically on a clean tree, so they are pre-existing baseline drift. Explore's filters are FIXED as of 2026-09-09: all eight axes were read from Astro.url.searchParams in frontmatter, which under output:"static" runs at build time with every parameter empty, so the server returned byte-identical HTML for every query string. Filtering now runs client-side (apps/site/src/lib/exploreFilter.ts) with per-option result counts; explore.spec.ts has 10 tests that all fail against the old page. Also fixed: --cp-muted raised 46%->54% (it failed WCAG AA on four of the five grounds it is actually used on; the contrast suite only measured one). Also open: port the progenax/startrax cross-validation fixtures for the IMF and cluster models; the star-cluster dynamics demo; instructor bundles for cluster-census + stars-zams-hr; Explore hardening continued 2026-09-10: trimmed three quick filters that could not change the result (labs matched 19 of 19; lt10 and noMath duplicated a select); collapsed the three status vocabularies to `readiness` alone on cards/exhibit pages via lib/readiness.ts (14 of 19 demos read draft + experimental + content_verified:true simultaneously) and swapped Explore's Status filter for Classroom readiness; made the 37 declared misconceptions searchable with per-word matching, curly-punctuation folding and stopword removal, showing the matched misconception on the card. OPEN DECISIONS FOR ANNA: (1) no demo is `readiness: launch-ready`, so every card still carries a badge -- the code is ready for silence, the frontmatter pass is not done, and 18 of 19 still say content_verified:true while reading `experimental`; (2) demo `tags` are narrow (seasons is tilt/sunlight/insolation, so a search for "summer" or "solstice" finds nothing) -- widening tags is the lever for search recall, not more search machinery
