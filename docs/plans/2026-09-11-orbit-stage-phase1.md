...
|---|---|---|---|
| 0 | — | Branch `claude/orbit-stage`, tree clean except `?? apps/site/.gitignore`. physics `conservationLawsModel.test.ts` 38/38 EXIT=0; theme 141/141 EXIT=0; demos `conservation-laws` 117/117 EXIT=0. Before-screenshots: the `9d446e4` captures in `output/playwright/conservation-laws/after/`. | — |
| 1 | e9949ba | RED: 8 new tests failed on `... is not a function` (TypeError inside `it`), 38 older passed, EXIT=1. GREEN: 46/46 EXIT=0. `tsc` TSC_EXIT=0. | None. |
| 2 | c044b56 | RED: 10 failed on assertions (3 alias `toMatch`, 7 `--cp-instr-glass-bg is defined`), EXIT=1. GREEN: `tokens` + `tokenContrast` 74/74 EXIT=0. BUILD_EXIT=0. Glass ground at 62% resolves to rgb(31, 32, 47); measured ratios: text 12.42, muted 5.16, amber 9.44, ice 11.62, kinetic 9.63, potential 5.91, total 12.42. | 62% kept; no opacity change needed. |
| 3 | 98ce792 | RED: 5 failed on assertions (empty `ruleBody`, `-1 > 10341`), EXIT=1. GREEN: `orbitShell` 5/5; full theme suite 156/156 EXIT=0. BUILD_EXIT=0. | None. |
| 4 | 14ed54f | First RED was not a RED: "Tests: no tests", EXIT=1, because the plan's tests call the helpers at `describe` scope, so the TypeError failed collection. Test fixed (see errata), then RED: 15 failed inside `it` on `is not a function`, 76 older passed, EXIT=1. GREEN: demo conservation-laws 132/132 EXIT=0; `apps/demos typecheck` TSC_EXIT=0. `formatNumber(x, 2)` checked: plain `toFixed` for abs(x) >= 1e-3, no zero trimming. | **Erratum 1:** `near`/`far` and `plot` moved from `describe` scope into `layouts()` / `makePlot()` factories called inside each `it`; assertions unchanged. **Erratum 2:** Step 2's "117 existing tests" is the whole conservation-laws directory; `logic.test.ts` alone had 76. |
| 5 | c1154c1 | RED: 8 failed on assertions (7 new markup tests + V6), 41 passed, 2 todo, EXIT=1. GREEN: 140 passed + 2 todo, EXIT=0. MATH_EXIT=0. `grep -c KEEP` = 0. ids 46 -> 63: none removed, the 17 new ids added. Trimmed-line diff against `124928d`: the only old lines gone are the two `aside` tags, the controls header, the orbit-type readout card, the two renamed labels, the Presets label, the intro sentence and `data-shell="triad"`. | **Erratum 3:** the markup renames "Specific energy" to "Total" (design section 7), but the existing V6 contract required "Specific energy". The plan said older contracts would still pass. The V6 regex now matches `Total ${\varepsilon = K + U}$`, still checking one KaTeX group. **Erratum 4:** "the eight new markup tests FAIL" is seven: "keeps the four regions" already passed on the old HTML. **Erratum 5:** Step 4's grep found two stale E2E checks in `conservation-laws.spec.ts`: `.cp-panel-header` first contains "Conservation Laws" (the dock has no header now, so it asserts `#cp-demo h1`) and `data-shell="triad"` (now `orbit`). Updated in this commit; not yet run (E2E starts in Task 7). "Specific angular momentum" is now "Angular momentum $|h|$", per design section 7, for the physics review to confirm. |
| 6 | 246bd3d | RED: the 2 styling tests failed on assertions (no `var(--cp-energy-kinetic)`; `.orbit` rule has `border`), 49 passed, EXIT=1. After the stylesheet: 141 passed, 1 failed (particle contract), EXIT=1. BUILD_EXIT=0. After the test fix: 142/142 EXIT=0. Mutation check: a `color-mix` put inside `.orbit__particle` makes the scoped test fail; the old regex was already true on the clean stylesheet. | **Erratum 6:** the existing contract "particle uses --cp-celestial-earth, not legacy color-mix" used `/\.orbit__particle[\s\S]*?color-mix/`, which matches any `color-mix` after the particle rule, here `.ueff__allowed`. It is now scoped to the rule body. **Heads-up for Task 8:** Step 5(c) takes `.orbit` out of the z-index rule, which will break the V5 contract regex `/\.orbit,\s*\.stage__caption\s*\{.../`; that test will need to follow `.stage__view`. |
| 7 | 6caa82b | RED: contract 1 failed on assertion, EXIT=1. E2E `-g "orbit shell and energy instrument"` on the Task 6 build: 3 failed on assertions (bar width 0, `data-radial-kinetic` 0, label `style.left` NaN), 3 passed, E2E_EXIT=1. GREEN: demo unit 143/143 EXIT=0; TSC_EXIT=0; BUILD_EXIT=0. Full `conservation-laws.spec.ts`: first run 79 passed, 8 failed, 3 skipped; after the test fixes below, 82 passed, 5 failed, 3 skipped, E2E_EXIT=1 (the 5 are the fold checks, see erratum 10). Marker test alone `--repeat-each=10`: 10/10, EXIT=0. | **Erratum 7:** "radial motion hides the energy bar and the plot" passed before the wiring, because Playwright counts a zero-size box as hidden; it now asserts both visible at the circular start first, and was RED on that (`toBeVisible`, received hidden). **Erratum 8:** older E2E encoded the triad layout: four fold checks expected 8 `.cp-readout` cards (now 7; the orbit type is the stage chip) and V4 expected the caption beside the orbit (design section 4 puts it under the drawing: now "within 40 px under, overlapping horizontally"). Both passed after the update. **Erratum 9:** "K ends on the total-energy marker" read the bars 41.6 px off right after the preset click. Diagnosed, not guessed: `style.left` was already Elliptical's 148.3px while the box was at Circular's 106.7px, and `document.getAnimations()` listed CSS transitions on `energyBarU/K:left,width` and `energyBarEps:left`. Cause: the theme's reduced-motion override sets `transition-duration: 0.01ms !important` on `*`, and `transition-property` defaults to `all`. Two animation frames settled it alone but not in the loaded full run, so `read()` now awaits the CSSTransitions' `finished`. Lead for the visual review, not fixed here: that override makes every style change a one-frame transition site-wide under reduced motion. **Erratum 10 (open, for Task 10 and Anna):** fold checks still fail. Settled at 1440x900 the lowest readout ends at 903.3 px (circular) and 954.9 px (radial); at 1280x720 at 925.2 px (circular) and 954.9 px (radial); V1's buttons end at 848.5 px at 1280x720. Design section 4 requires only the stage, dock and energy bar above the fold at 1280x720, so "all readouts above the fold at 1280x720" conflicts with the approved design. |
| 8 | f556afe (Step 2), 2bc2611 | Step 1 RED: 7 failed inside `it` on `is not a function`, 91 passed, EXIT=1. Step 2 GREEN: demo unit 150/150 EXIT=0 (includes every Task 4 `effectivePotentialPlot` test after the refactor); TSC_EXIT=0. Step 3 RED: contracts 2 failed on assertions, 52 passed, EXIT=1; E2E (after BUILD_EXIT=0) 2 failed: `data-view` received "" (assertion) and the Potential tab absent (locator timeout, accepted by the plan). Step 7 GREEN: demo unit 152/152 EXIT=0; TSC_EXIT=0; MATH_EXIT=0; BUILD_EXIT=0. Full `conservation-laws.spec.ts`: 83 passed, 6 failed, 3 skipped; after the test fix below, the two view tests `--repeat-each=3` 6/6 EXIT=0, leaving only the 5 fold checks of erratum 10. | **Erratum 11:** `potentialProfile` tests written with a `makeProfile()` factory from the start, since the plan's describe-scope call would fail collection as in erratum 1. **Erratum 12 (the Task 6 heads-up):** Step 5(c) took `.orbit` out of the stacking rule, so V5 now asserts `.stage__head, .stage__caption` and `.stage__view` each at `position: relative; z-index: 1`. **Erratum 13:** "switches ... without moving the body" compared the dock's viewport `top` before and after the tab click and read a 460 px move: Step had scrolled the page down to the dock (scrollY 460, the same value erratum 9's diagnostic recorded) and clicking the tab scrolled it back. It now compares page coordinates (`top + scrollY`). |
| 9 | 5e00511 | Plan's route check: the existing instructor test uses `instructor/conservation-laws/`, and `webServer` is `pnpm preview` of the built site. RED: contract 1 failed on assertion (`What to notice` lacks "Potential"), 54 passed, EXIT=1; E2E 1 failed (`getByText('Turning points:')` not found), E2E_EXIT=1. GREEN: demo unit 153/153 EXIT=0; MATH_EXIT=0; BUILD_EXIT=0; `STATION_COPIES_MATCH` printed. Full `conservation-laws.spec.ts`: 85 passed, 5 failed, 3 skipped, E2E_EXIT=1; the 5 are erratum 10's fold checks, with the same measurements after Task 8 (903.3, 925.2, 954.9 px; V1 848.5 px). | **Erratum 14:** the station E2E matched `/Switch the stage to\s+Potential/`, case-sensitive, but the plan's card copy reads "Then switch the stage to **Potential**". The copy is kept; the regex is now `/switch the stage to\s+Potential/i`. |
| 10 | — (stopped at Step 2) | Anna's decision on erratum 10: keep all readouts above the fold at 1440x900, replace the 1280x720 all-readouts checks with the plan's stage/energy-bar/dock fold test (both edits made in `conservation-laws.spec.ts`, uncommitted). Step 1 RED: the fold test failed on the dock at both sizes, bottom 1106.9 px at 1440x900 and 1055.5 px at 1280x720, E2E_EXIT=1. Step 2 measurement (temporary probe spec, deleted, never committed; reduced motion, fonts ready, root font 18px, `#cp-demo` padding 16px, row gap 12px): **1280x720**: instrument column 24 to 1045.5 px (1021.5 tall: header 52, energy section 337.6, U_eff section 335, values 214.9, two 24px gaps, 16px padding); stage 24 to 629.6; dock 766.8 to 1045.5 (278.7 tall), leaving a 137 px empty band between stage and dock. Stage-budget sweep 16 to 30 rem (view 432 px down to the 270 px floor): dock bottom 1045.5 px at every value. **1440x900**: instrument 24 to 997.8 (973.8 tall); lowest readout bottom 980.8 (circular), 954.9 (radial); dock 309.1 tall; sweep: dock bottom 1096.9 (16rem), 1060.9 (18), 1024.9 (20), then 997.8 at 22 to 30 rem, i.e. pinned to the instrument's bottom. | **STOP (plan Task 10 Step 2):** no rem at or above the 270 px floor passes at 1280x720, nor at 1440x900. Cause: `.cp-demo__readouts` spans grid rows 1 and 2 ("viz readouts" / "sidebar readouts"), so those two rows together are at least the instrument's height and the dock's bottom cannot rise above the instrument's bottom; the stage budget only redistributes height inside that span. Reported to Anna with options; layout not changed. **Anna chose option A** (decouple and trim): cap the instrument at the viewport, trim it so 1440x900 needs no inner scroll, and at 1280x720 require the stage view, energy bar and Play/Step row rather than the whole dock. |
| 10 (option A) | 0779f2b | Second probe (temporary, deleted), cumulative CSS injections, dock/readout bottoms in px. 1440x900: cap + 27rem gives dock 898.9, instrument scroll 104; + plot 7.5rem scroll 77; + body gap space-4 scroll 61; + readout margins 0 scroll 0, lowest readout 866.1; + values gap space-2/3 lowest readout 836.2. 1280x720: cap + 27rem gives view 270 (floor), Play/Step row 556.2, dock 758.3, instrument scroll 332 (213 with all trims). Source of the stray spacing: `stub-demo.css` `.cp-readout { margin-top: 12px }` plus the label's 4px. RED: theme `orbitShell` 4 failed on assertions, 5 passed, EXIT=1; E2E fold tests 2 failed (dock 1106.9 at 1440x900; Play/Step row 853.4 at 1280x720), E2E_EXIT=1. GREEN: theme 160/160 THEME_EXIT=0; demo unit 153/153; runtime 36/36 RUNTIME_EXIT=0, RUNTIME_TSC_EXIT=0; BUILD_EXIT=0. Step 3 (`conservation-laws`, `layout-budget`, `reflow`, `accessibility`, `smoke` in one run): first 468 passed, 1 failed, 3 skipped; after the test fix below 469 passed, 3 skipped, E2E_EXIT=0. | **Erratum 15 (deviation, flagged):** the stage budget is 28rem, not the smallest-passing 27rem: 27rem left the dock 1.1px inside the fold at 1440x900, which Linux CI's taller fonts would break (layout-budget notes). **Erratum 16:** option A needed a visible cue for the instrument's inner scroll at 1280x720, so `initScrollAffordance`'s selector now includes `.cp-demo[data-shell="orbit"] .cp-demo__readouts .cp-panel-body` and `demo-shell.css` gives it the controls' edge fade; E2E asserts `data-scroll="bottom"` there. **Erratum 17:** the 1280x720 fold test first read the instrument at top 34px against the stage's 24px (bottom 724px): the entry animations' stagger delay (50ms steps) is not shortened by the reduced-motion override, so `both` fill held `cp-slide-up`'s 10px offset. The describe's "reduced motion switches off the entry slide" was wrong; both fold tests now wait for finite animations. Lead for the visual review, not fixed: under reduced motion panels still appear 10px low for up to 150ms, then jump. |
| 10 (visual review, Steps 4-6) | ceadf57 | One `visual-ux-reviewer` on 0779f2b; captures in `output/playwright/conservation-laws/orbit-shell/` (not committed). Passed: 1440x900 dock 880.9px, no inner scroll, view 396px; 1280x720 view/bar/Play-Step row at 385.8/206.2/556.2px, dock 758.3px, instrument scrolls 213px with the fade; K end meets the eps marker within 0.0-0.1px at four sizes over 12 Steps; r_p/r_a labels within 0.3px of the crossings, 34.7px apart, 12.6px from the curve; focus visible on tabs, presets and Step; 18 of 18 controls act; no sideways scroll at 320px; phone order stage, instrument, dock, drawer. **High (fixed):** the sticky instrument slid over the drawer when scrolled, 346x443px at 1440x900 and 307x504px at 1280x720. RED: new E2E overlap test failed at both sizes on the assertion (instrument 16-845px over drawer 402-876px at 1440x900), E2E_EXIT=1. Fix: not sticky. GREEN: theme 160/160; BUILD_EXIT=0; Step 3 run 471 passed, 3 skipped, E2E_EXIT=0. | **Deferred (Medium):** (1) `#potentialEpsLabel` at `box.width - 12` (main.ts) touches the rising U_eff branch, 0px at 1280x720, 390x844, 320x640; (2) text on the open stage (caption, title) drops to 1.0-2.8:1 where a star falls under a glyph, and the Potential tab reads 3.3:1 at 390px; glass panels pass (muted labels at or above 5.37:1). **Deferred (Low):** (3) reduced motion: entry panels hidden for the 50-150ms stagger delay, and 0.01ms `all` transitions leave bar `left` stale for one frame (theme-wide); (4) at 320x640 switching views moves the dock and drawer 30.4px because the status line wraps from 2 to 3 lines; (5) `p.instrument__equation` (`overflow-x: auto`) is an unlabelled Tab stop; (6) KaTeX breaks drawer inequalities across lines at 390 and 320px; (7) the energy bar's 4px radius leaves about 3px notches beside the marker; the mirrored left half of the profile is unlabelled. |
| 11 (round 1) | 86c63d3, d12f81c, 90b3b61, 85bc080 | `physics-reviewer` on ceadf57: model and logic recomputed to 6 figures (asymmetric state and Elliptical after 3 Steps: K + U = eps, eps - U_eff = (sqrt(mu/p) e sin nu)^2/2, Step 8 lands on r_p with v_r^2/2 = 0); energy bar, y direction, "Circular: touches the bottom" (circular requires e < 1e-10) and all reader copy verified correct. Verdict **NOT SAFE**. RED for each fix, all on assertions: (1) logic 2 failed, first curve y 74.58/150 and 149.16/300 for a circular toy; (3) physics 1 failed, NaN at r_p for 7 of 45 exact-escape grid orbits; (2) E2E 1 failed, `#ueffDot` visible at apoapsis r_a = 49 AU in a 6 AU view; (5) contract 1 failed on `h^2/2r^2`. GREEN: physics 47/47, PHYS_TSC_EXIT=0; demo 156/156, DEMO_TSC_EXIT=0; MATH_EXIT=0; BUILD_EXIT=0; `conservation-laws` + `layout-budget` + `reflow` + `accessibility` + `smoke` 472 passed, 3 skipped, E2E_EXIT=0 (fold tests hold with the longer label); STATION_COPIES_MATCH. | Findings: **(1) verified**, curves started at 0.55 r_p, above the window's top only for e >= 0.212, so Circular drew a well open towards the Sun: start is now min(0.55 r_p, the r where U_eff reaches the top), by bisection on the injected U_eff (86c63d3). **(3) lead, confirmed by RED:** tolerance now 1e-9 max(abs(eps), mu/r) (d12f81c). **(2) lead, confirmed by RED:** dot and drop line hidden while r exceeds the view radius (90b3b61). **(5) lead:** `h^2/(2r^2)`, `-\mu^2/(2h^2)` in the drawer and the MW Short; label "Angular momentum per unit mass $|h|$" (85bc080). **(4) lead, deferred:** neither potential drawing states its x-scale (the plot starts at the curve start, the profile is linear from the Sun); design section 5.3 schedules scale labels with the Phase 2 label overlay, and a caption line would push the instrument into scrolling at 1440x900. |
| 11 (round 2) | — | `physics-reviewer` on `git diff ceadf57..85bc080`, recomputed independently: (1) dU_eff/dr = (mu/r^3)(r - h^2/mu) < 0 on (0, 0.55 r_p]; the start moves in only for e < 0.2123 (Circular now starts at 0.4537 AU, y = 0.00; speed 1.05 at 0.5002 r_p; Elliptical, speed 0.9 at 30 deg, Escape and Hyperbolic unchanged at 0.55 r_p; E2E label test's assumption holds, 0 px); (2) sweep of 108 open and many bound orbits: no in-view position hidden, none beyond 0.1% outside shown, view-edge overshoot at most 1.6e-15; (3) 1350 orbits: no NaN at 201 allowed radii, NaN at 0.5 r_p, 0.9 r_p, 1.1 r_a, 1.5 r_a, abs value <= 1e-9 mu/r at turning points; (5) no ambiguous fraction left in conservation-laws copy. Physics 47/47, demo 156/156. Verdict **SAFE TO SHIP**. | (4) accepted as a Phase 1 deferral: nothing reader-visible mislabels distance; caveat, near-circular and other orbits now have different plot left edges until the scale label lands. Minor, not blocking, deferred: the station card's word bank (stations/conservation-laws.md:27 and its copy in activities.md) still says "Angular momentum $h$" without "per unit mass". |
| 12 | (this commit) | `corepack pnpm gates` GATES_EXIT=0: lint EXIT=0 (1s), typecheck EXIT=0 (7s), unit EXIT=0 (9s; main vitest run 48 files, 1,896 tests, plus five small suites of 5, 2, 3, 2, 3), build EXIT=0 (7s), e2e EXIT=0 (198s; 1,204 passed, 34 skipped, 0 failed). Handoff screenshots after Elliptical + 3 Steps, both views, via a temporary spec (deleted, not committed): `output/playwright/conservation-laws/orbit-shell/final-{observatory,potential}-{1440x900,390x844}.png` plus `-full` for 390x844. STATUS.md updated. `apps/site/src/content/demos/conservation-laws.md` status and readiness unchanged (beta / candidate); `last_updated` was not touched. | Not pushed; waiting for Anna's approval to land. |
