...
- Animation: legacy advanced the true anomaly with fixed Euler sub-steps of dnu/dt = h/r^2, 0.02 s of screen time each, with the frame gap capped at 0.1 s (`conservation-laws.js:278-301`). The port steps Kepler's equation for bound orbits, so their timing does not depend on the frame rate or step size, and integrates dnu/dt = h/r^2 for open orbits in sub-steps capped at 0.01 rad and at a 0.1% change in the rate (`advanceTrueAnomalyByTime` in `packages/physics/src/conservationLawsModel.ts`). The time scale is unchanged at 1/3 yr per second. New: Play restarts an open orbit that has left the view.
...
- View: legacy fitted the drawn path; now a window continuous across e = 1, capped at 6 r0 and kept between 1.5 r0 and 50 AU.
...
- Animation stepping: Kepler's equation for bound orbits and rate-capped sub-steps for open ones, in place of legacy's fixed Euler sub-steps, as above.
